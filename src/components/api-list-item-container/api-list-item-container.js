        ...
        return (
            <div className="list-item">
                <div className="avatar-container">
                    <img src={ get(commit, 'author.avatar_url') } alt={ get(commit, 'author.login') } title={ get(commit, 'author.login') } />
                </div>
                <div className="info-container">
                    <h4 title={ get(commit, 'commit.message') }>{ get(commit, 'commit.message') }</h4>
                    <p>{ Moment(get(commit, 'commit.author.date')).format(APP_CONSTANTS.FORMAT.DATE) }</p>
